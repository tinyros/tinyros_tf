  ...
  class ImageMarker : public tinyros::Msg
  {
    ...
    void deconstructor()
    {
      ...
      if(outline_colors != NULL)
      {
        for( uint32_t i = 0; i < outline_colors_length; i++){
          outline_colors[i].deconstructor();
        }
        free(outline_colors);
      }
      outline_colors = NULL;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(outline_colors_lengthT > outline_colors_length)
        this->outline_colors = (tinyros::std_msgs::ColorRGBA*)realloc(this->outline_colors, outline_colors_lengthT * sizeof(tinyros::std_msgs::ColorRGBA));
      outline_colors_length = outline_colors_lengthT;
      for( uint32_t i = 0; i < outline_colors_length; i++) {
        offset += this->st_outline_colors.deserialize(inbuffer + offset);
        memcpy( &(this->outline_colors[i]), &(this->st_outline_colors), sizeof(tinyros::std_msgs::ColorRGBA));
      }
      return offset;
    }
    ...
    virtual tinyros::string getType(){ return "visualization_msgs/ImageMarker"; }
    static tinyros::string getTypeStatic(){ return "visualization_msgs/ImageMarker"; }
    virtual tinyros::string getMD5(){ return "453bce92c8194bc4489b79af1cdf861e"; }
    static tinyros::string getMD5Static(){ return "453bce92c8194bc4489b79af1cdf861e"; }
