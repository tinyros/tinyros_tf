  ...
  class Log : public tinyros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (this->line >> (8 * 3)) & 0xFF;
      ...
      uint32_t topics_length = this->topics.size();
      *(outbuffer + offset + 0) = (topics_length >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (topics_length >> (8 * 1)) & 0xFF;
      *(outbuffer + offset + 2) = (topics_length >> (8 * 2)) & 0xFF;
      *(outbuffer + offset + 3) = (topics_length >> (8 * 3)) & 0xFF;
      offset += sizeof(topics_length);
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += sizeof(this->line);
      uint32_t topics_length = ((uint32_t) (*(inbuffer + offset))); 
      topics_length |= ((uint32_t) (*(inbuffer + offset + 1))) << (8 * 1); 
      topics_length |= ((uint32_t) (*(inbuffer + offset + 2))) << (8 * 2); 
      topics_length |= ((uint32_t) (*(inbuffer + offset + 3))) << (8 * 3); 
      this->topics.resize(topics_length); 
      offset += sizeof(topics_length);
      for( uint32_t i = 0; i < topics_length; i++) {
