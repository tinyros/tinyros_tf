...
def primitiveType(ty):
    if ty in ['bool', 'int8_t', 'char', 'uint8_t', 'int16_t', 'uint16_t', 'int32_t', 'uint32_t', 'int64_t', \
             'uint64_t', 'float', 'double', 'tinyros::Time', 'tinyros::Duration', 'std::string']:
        return True
    else:
        return False
...
class Message:
    ...
    def _write_getType(self, f):
        ...
        f.write('    static std::string getTypeStatic(){ return "%s/%s"; }\n'%(self.package, self.name))

    def _write_getMD5(self, f):
        f.write('    virtual std::string getMD5(){ return "%s"; }\n'%self.md5)
        f.write('    static std::string getMD5Static(){ return "%s"; }\n'%self.md5)

    def _write_getDefinition(self, f):
        f.write('    virtual std::string getDefinition(){ return "%s"; }\n'%self.definition)
        f.write('    static std::string getDefinitionStatic(){ return "%s"; }\n'%self.definition)
...
class Service:
    ...
    def make_header(self, f):
        ...
        def write_type(out, name):
            ...
            out.write('    static std::string getTypeStatic(){ return %s; }\n'%(name))
