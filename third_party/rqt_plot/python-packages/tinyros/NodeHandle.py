...
class NodeHandleBase(object):
    ...
    __slots__ = ['ip_', 'name_', 'spin_', 'publishers_', 'subscribers_', 'hardware_', 'loghd_', 'keepalive_', 'keepalive_thread_', 'mutex_', 'message_class_cache_']
...
class NodeHandle(NodeHandleBase):
    ...
    def getTopicList(self, timeout = 1000):
        ...
        topic_list = []
        topic_split = [t for t in self.topic_list_.split('\n') if len(t) > 0]
        for t in topic_split:
            name = t[:t.find(' [')]
            type = t[t.find('type:') + 5 : t.find(',')]
            topic_list.append((name, type))
        return topic_list
    ...
    def package_resource_name(self, name): 
        if '/' in name:
    ...
    def get_message_class(self, message_type):
        if message_type in self.message_class_cache_:
            return self.message_class_cache_[message_type]
        if message_type == 'time':
            return Time
        if message_type == 'duration':
            return Duration
        ## parse package and local type name for import
        package, base_type = self.package_resource_name(message_type)
        if not package:
            if base_type == 'Header':
                package = 'std_msgs'
            else:
        ...
        try:
            # import the package
            pypkg = __import__('%s.msg' % package)
        except ImportError:
            # try importing from dry package if available
            print("import package(%s.msg) failed" % package);
        if pypkg:
            try:
                val = getattr(getattr(pypkg, 'msg'), base_type)
            except AttributeError:
                pass
        if val:
            self.message_class_cache_[message_type] = val
        return val
