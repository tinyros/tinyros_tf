...
// Set the macAPI for Ogre based on the WX implementation
...
#if OGRE_PLATFORM == OGRE_PLATFORM_LINUX
  {
    ...
    if( rtt_preferred_mode )
    {
      ...
      {
        tinyros_log_error( "Environtment variable OGRE_RTT_MODE has invalid value: '%s'.  Should be", rtt_preferred_mode );
        tinyros_log_error( "either FBO, PBuffer, Copy, or should not be set.  Using default value %s.", rtt_default );
    ...
    tinyros_log_info( "RTT Preferred Mode is %s.", rtt_preferred_mode );
  }
#endif
